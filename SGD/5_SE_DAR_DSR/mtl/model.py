...
class SE_DAR_DSR(nn.Module):
    def __init__(self, args, vocab_size, class_num, pretrained_private=None, pretrained_encoder=None):
        ...
        if pretrained_encoder is not None:
            ...
            self.encoder = TransformerEncoder(d_model, d_model*3, 8, 2, 0.2)
        ...
        self.sat_seq_gru = nn.GRU(5, d_model, num_layers=1, bidirectional=False, batch_first=True)


        self.act_classifier = MLP(d_model, class_num, d_model//2)
        self.sat_seq_classifier = MLP(d_model, 5, d_model//2)
        self.sat_classifier = nn.Linear(d_model, 5)
        self.crf = torchcrf.CRF(class_num, batch_first=True)
        self.sat_seq_crf = torchcrf.CRF(5, batch_first=True)
        ...
        self.U_s = nn.Linear(d_model, d_model)
        self.w_s = nn.Linear(d_model, 1, bias=False)

        self.w = nn.Linear(d_model*3, 1)
        
        init_params(self.act_classifier)
        init_params(self.sat_seq_classifier)
        init_params(self.encoder)
        init_params(self.sat_classifier)
        init_params(self.w)
        ...
        init_params(self.U_s)
        init_params(self.w_s)

    def forward(self, input_ids, act_seq=None,sat_seq=None, sat=None, **kwargs):
        self.content_gru.flatten_parameters()
        self.act_gru.flatten_parameters()
        self.sat_seq_gru.flatten_parameters()

        batch_size, dialog_len, utt_len = input_ids.size()

        input_ids = input_ids.view(-1, utt_len)
        attention_mask = act_seq.ne(-1).detach() # 8,16
        attention_mask_sat_seq = sat_seq.ne(-1).detach() # 8,16
        ...
        H_sat_seq = self.encoder(private_out, attention_mask_sat_seq)
        H_sat_seq = self.drop_out(H_sat_seq) # 8, 16, 1536

        act_res = self.act_classifier(H) # 8, 16, 12
        sat_seq_res = self.sat_seq_classifier(H_sat_seq) # 8, 16, 7
        ...
        H, _ = self.sat_seq_gru(sat_seq_res)
        att_s = self.w_s(torch.tanh(self.U_s(H))).squeeze(-1)
        att_s = F.softmax(att_s.masked_fill(mask=~attention_mask_sat_seq, value=-np.inf), dim=1)
        sat_seq_hidden = torch.bmm(H.permute(0,2,1), att_s.unsqueeze(-1)).squeeze(-1)

        hidden = torch.cat([content_hidden,act_hidden,sat_seq_hidden], dim=-1)
        pointer = torch.sigmoid(self.w(hidden))
        hidden = content_hidden * pointer + act_hidden * (1-pointer)/2 + sat_seq_hidden * (1-pointer)/2
        sat_res = self.sat_classifier(hidden)

        if self.training:
            #act_loss = F.cross_entropy(act_res.view(-1, self.class_num), act_seq.view(-1), ignore_index=-1)
            act_loss = -1*self.crf(act_res, act_seq, mask=attention_mask)
            sat_seq_loss = -1*self.sat_seq_crf(sat_seq_res, sat_seq, mask=attention_mask_sat_seq)
            weight_class1 = torch.FloatTensor([2102.6666666666665,12.107485604606525,1.0,6.536787564766839,161.74358974358975]).cuda()
            sat_loss = F.cross_entropy(sat_res, sat, weight=weight_class1)

            return act_res, sat_seq_res, sat_res, act_loss, sat_seq_loss, sat_loss

        return act_res, sat_seq_res, sat_res, pointer, att_a, att_c, att_s

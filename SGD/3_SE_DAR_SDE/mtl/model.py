...
class SE_DAR_SDE(nn.Module):
    ...
    def forward(self, input_ids, act_seq=None, sat_diff=None, sat=None, **kwargs):
        ...
        if self.training:
            ...
            weight_class2 = torch.FloatTensor([5.527977044476327,3.2791489361702126,1.0,2.6775538568450314,5.7336309523809526]).cuda()
            sat_diff_loss = F.cross_entropy(sat_diff_res, sat_diff, weight=weight_class2)
            
            weight_class1 = torch.FloatTensor([2.064534231200898, 1.5490526315789475, 1.0]).cuda()

...
from model import SE_DAR_SDE, Pretrained_HiTrans
...
class DataFrame(Dataset):
    def __init__(self, data, args):
        ...
        self.sat_diff = data['sat_diff']
        self.sat = data['sat']
        self.max_len = args.max_dia_len
    
    def __getitem__(self, index):
        return self.input_ids[index][-self.max_len:], self.act_seq[index][-self.max_len:], self.sat_diff[index], self.sat[index]
...
def collate_fn(data):
    input_ids, act_seq, sat_diff, sat = zip(*data)
    ...
    return {'input_ids':  input_ids,
            'act_seq': act_seq,
            'sat_diff': torch.tensor(sat_diff).long(),
...
def train(args):
    ...
    if model_name == 'SE_DAR_SDE':
        model = SE_DAR_SDE(args=args, vocab_size=tokenizer.vocab_size, class_num=act_num, pretrained_private=pretrained_private, pretrained_encoder=pretrained_encoder)
    optimizer = AdamW(model.parameters(), 2e-5)

    
    act_best_result = [0. for _ in range(4)]
    sat_diff_best_result = [0. for _ in range(4)]
    ...
    for i in range(args.epoch_num):
        ...
        for j, batch in enumerate(tk0):
            ...
            sat_diff = batch['sat_diff'].to(args.device)
            sat = batch['sat'].to(args.device)

            act_pred, sat_diff_pred, sat_pred, act_loss, sat_diff_loss, sat_loss = model(input_ids=input_ids, act_seq=act_seq, sat_diff=sat_diff, sat=sat)

            if len(args.device_id) > 1:
                act_loss = act_loss.mean()  # mean() to average on multi-gpu parallel training
                sat_diff_loss = sat_diff_loss.mean()
                sat_loss = sat_loss.mean()

            epoch_loss[0] += sat_loss
            epoch_loss[1] += act_loss
            epoch_loss[2] += sat_diff_loss

            loss = 0.01*act_loss + sat_loss + 0.01*sat_diff_loss
        ...
        act_test_result, sat_diff_test_result, sat_test_result = test(model, DataFrame(data['valid'], args), args)
        if sat_test_result[-1] > sat_best_result[-1]:
            sat_best_result = sat_test_result
            sat_diff_best_result = sat_diff_test_result
            act_best_result = act_test_result
            model_to_save = model.module if hasattr(model,
                        'module') else model  # Take care of distributed/parallel training
            torch.save(model_to_save.state_dict(), save_path+'/best_pretrain.pt')
        logging.info(f'satisfaction: valid_result={sat_test_result}, action: valid_result={act_test_result}, satisfaction diff: valid_result={sat_diff_test_result}')
        logging.info(f'satisfaction: best_valid_result={sat_best_result}, action: best_valid_result={act_best_result}, satisfaction diff: best_valid_result={sat_diff_best_result}')
        act_test_result, sat_diff_test_result, sat_test_result = test(model, DataFrame(data['test'], args), args)
        logging.info(f'satisfaction: test_result={sat_test_result}, action: test_result={act_test_result}, satisfaction diff: test_result={sat_diff_test_result}')
...
def test(model, test_data, args):
    ...
    for j, batch in enumerate(tk0):
        ...
        sat = batch['sat'].to(args.device)
        with torch.no_grad():
            act_pred, sat_diff_pred, sat_pred, _, _, _ = model(input_ids=input_ids, act_seq=act_seq)
        act_prediction.extend(act_pred.argmax(dim=-1).cpu().tolist())
        sat_diff_prediction.extend(sat_diff_pred.argmax(dim=-1).cpu().tolist())
        sat_prediction.extend(sat_pred.argmax(dim=-1).cpu().tolist())
        act_label.extend(act_seq.cpu().tolist())
        sat_diff_label.extend(sat.cpu().tolist())
    ...
    # satisfaction diff evaluation
    acc = sum([int(p == l) for p, l in zip(sat_diff_prediction, sat_diff_label)]) / len(sat_diff_label)
    precision = precision_score(sat_diff_label, sat_diff_prediction, average='macro', zero_division=0)
    recall = recall_score(sat_diff_label, sat_diff_prediction, average='macro', zero_division=0)
    f1 = f1_score(sat_diff_label, sat_diff_prediction, average='macro', zero_division=0)
    sat_diff_result = (acc, precision, recall, f1)

    return act_result, sat_diff_result, sat_result
...
def evaluate(args):
    ...
    if model_name == 'SE_DAR_SDE':
        model = SE_DAR_SDE(args=args, vocab_size=tokenizer.vocab_size, class_num=act_num)
    ...
    for j, batch in enumerate(tk0):
        ...
        sat = batch['sat'].to(args.device)
        with torch.no_grad():
            act_pred, sat_diff_pred, sat_pred, pointer, att_a, att_c = model(input_ids=input_ids, act_seq=act_seq)
        act_prediction.extend(act_pred.argmax(dim=-1).cpu().tolist())
        sat_diff_prediction.extend(sat_diff_pred.argmax(dim=-1).cpu().tolist())
        sat_prediction.extend(sat_pred.argmax(dim=-1).cpu().tolist())
        act_label.extend(act_seq.cpu().tolist())
        sat_diff_label.extend(sat_diff.cpu().tolist())
        sat_label.extend(sat.cpu().tolist())
    ...
    logging.info(f'satisfaction: test_result={sat_result}, action: test_result={act_result}, satisfaction diff: test_result={sat_diff_result}')

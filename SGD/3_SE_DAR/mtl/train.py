...
from model import SE_DAR, Pretrained_HiTrans
...
def train(args):
    ...
    if model_name == 'SE_DAR':
        model = SE_DAR(args=args, vocab_size=tokenizer.vocab_size, class_num=act_num, pretrained_private=pretrained_private, pretrained_encoder=pretrained_encoder)
    ...
    for i in range(args.epoch_num):
        ...
        logging.info(f'satisfaction: valid_result={sat_test_result}, action: valid_result={act_test_result}')
        logging.info(f'satisfaction: best_valid_result={sat_best_result}, action: best_valid_result={act_best_result}')
        act_test_result, sat_test_result = test(model, DataFrame(data['test'], args), args)
        logging.info(f'satisfaction: test_result={sat_test_result}, action: test_result={act_test_result}')
...
def evaluate(args):
    ...
    if model_name == 'SE_DAR':
        model = SE_DAR(args=args, vocab_size=tokenizer.vocab_size, class_num=act_num)
    ...
    logging.info(f'satisfaction: test_result={sat_result}, action: test_result={act_result}')

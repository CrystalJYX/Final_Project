...
class SE_DAR_DSR(nn.Module):
    def __init__(self, args, vocab_size, class_num, pretrained_private=None, pretrained_encoder=None):
        ...
        if pretrained_encoder is not None:
            ...
            self.encoder = TransformerEncoder(d_model, d_model*3, 8, 2, 0.1)
        ...
        self.sat_seq_gru = nn.GRU(3, d_model, num_layers=2, bidirectional=False, batch_first=True)


        self.act_classifier = MLP(d_model, class_num, d_model//2)
        self.sat_seq_classifier = MLP(d_model, 3, d_model//2)
        self.sat_classifier = nn.Linear(d_model, 3)
        self.crf = torchcrf.CRF(class_num, batch_first=True)
        self.sat_seq_crf = torchcrf.CRF(3, batch_first=True)
    ...
    def forward(self, input_ids, act_seq=None,sat_seq=None, sat=None, **kwargs):
        ...
        if self.training:
            ...
            weight_class = torch.FloatTensor([1.4405263157894737, 1.0, 1.248631386861314]).cuda()
            sat_loss = F.cross_entropy(sat_res, sat, weight=weight_class)
